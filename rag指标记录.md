...
OCR 端到端的损耗在电子版和扫描难题上差异巨大，不应笼统看一个 38%。
脚本：`tests/eval/eval_ocr_by_type.py`

| 文档类型 | 题数 | Hit@1 | Hit@3 | Hit@5 | MRR |
|----------|:----:|:-----:|:-----:|:-----:|:---:|
| 电子版（font/long/wide） | 28 | 0.43 | **0.57** | 0.57 | 0.488 |
| 扫描难题（handwriting/high_pixel/watermark等） | 20 | 0.15 | **0.15** | 0.15 | 0.150 |
| 加权平均 | 48 | — | 0.40 | — | — |

**分层分析**：

- **电子版 Hit@3=57%**：远高于扫描难题的 15%。这类文档（Word/PDF 导出的产品手册、规范、报告）文字层是真实的，MinerU 直接提取近无损，损耗主要来自表格 HTML 结构差异和命中判定的精确子串匹配。
- **扫描难题 Hit@3=15%**：手写体/高噪声扫描件 OCR 识别误差大（别字、漏字、内容缺失），evidence 原文在 MinerU 解析结果中往往不存在，检索自然无法命中。
- **企业真实场景参考**：企业知识库以电子版文档为主（占比通常 70%+），实际端到端表现会**接近电子版的 57%，远好于整体的 38%**。扫描难题（手写/模糊件）占比小且属于已知边界。

> **注意**：即便电子版的 57% 也低于标准标注的 94%，这是命中判定逻辑（evidence 精确子串匹配）的局限——MinerU 输出的表格 HTML 结构、排版细节与人工标注存在格式差异，导致子串匹配失败。这不代表检索失败，而是评测口径偏严。若用语义相似度判定，数字会更高。
